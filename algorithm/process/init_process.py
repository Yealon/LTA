...
def init_all(config, gpu_mode, checkpoint, mode, *args, **params):
    ...
    '''
    step2. 初始化train，valid，test对应的formatter，方便后续get_formatter
    '''
    logx.msg(infor_msg("Use router to initialize formatter..."))
    if mode == "train":
        route_formatter(config, ["train", "valid"], *args, **params)
    else:
        route_formatter(config, ["test"], *args, **params)
    '''
    step3. 加载checkpoints，若有指定，分别加载存储的model，trained_epoch，若为训练模式，继续加载optimizer，global_step
            最后加载报告函数report_function
    '''
    ...
    result["report_function"] = init_report_function(config)
