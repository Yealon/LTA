<template>
  <div>
    <s-header title="云心网络隐私条款"></s-header>
    <div id="list-main-wrap">
      ...
      >
        <h4 class="list-box__title">
          {{ resource.title }}
          <span class="host" v-if="resource.host">({{ resource.host }})</span>
        </h4>
        <ul class="list-box__info">
          <li class="list-box__info__meta">{{ resource.user.username }}</li>
          <li class="list-box__info__meta">
            <timeago
              :since="resource.ctime"
              :max-time="86400 * 365"
              :format="formatTime"
            ></timeago>
          </li>
          ...
          <li
            class="list-box__info__meta list-box__info__meta_extra"
            @click="jumpDetailComment(resource)"
          >
            评论 {{ resource.cmtnum }}
          </li>
        </ul>
